...
const EditProduct = () => {
    ...
    axios
      ...
      .then((res) => {
        if (res.data.status) {
          ...
          setSelectedOptionCategory(res.data.data.category);
  ...
  const PostVariants = (productID) => {
    ...
    for (let item in AttributesInputValue) {
      ...
      if (AttributesInputValue[item]["variant_price"] === undefined || AttributesInputValue[item]["variant_price"] === "") {
        ...
        singleData["price"] = parseInt(AttributesInputValue[item]["variant_price"]);
        // data.append("price", parseInt(productAllData["product_price"]) + parseInt(AttributesInputValue[item]["variant_price"]));
      }
      singleData["stock"] = parseInt(AttributesInputValue[item]["variant_stock"]);
  ...
  console.log("discount", productAllData["product_discount"]);
  ...
  const postProduct = () => {
    ...
    selectedOptionCategory != null && data.append("category", selectedOptionCategory);
  ...
  console.log("AttributesInputValue", AttributesInputValue);
  console.log(selectedOptionCategory?.value, "==================================OptionCategorry==============");
  useEffect(() => {
    getAttributeList();
  }, []);
            ...
            <div className="col-12 col-md-6">
              ...
              <InputGroup className="mb-3">
                <Form.Control
                  ...
                  aria-describedby="basic-addon1"
                  style={{ backgroundColor: "#FAFAFA" }}
                  placeholder={`${singleProduct?.barcode_text}`}
                  disabled
                />
              </InputGroup>
            </div>
      ...
      <div className="product-main my-4 bg-white cat-child" style={{ borderTop: "0.5px solid #E0E0E0" }}>
        <div className="row d-flex justify-content-between">
          ...
          <div className="mt-2 mt-md-4" style={{ marginLeft: "-10px" }}>
            ...
            <button onClick={() => history.push(path.product_list)} className="save-btn mt-2">
              Cancel
            </button>
          </div>
        </div>
