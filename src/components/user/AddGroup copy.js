...
const AddGroup = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        for (let key in resultpermissions) {
          ...
          objMake[Capitalize(currentObj.content_type)] = { ...objMake[Capitalize(currentObj.content_type)], [appName]: true };
        }
        console.log("result - obj", objMake);

        var defaultOptionsMake = Object.keys(objMake).map((curr, index) => ({ value: index, label: curr }));
  ...
  const handleCheckBoxChange = (e, variantName) => {
    ...
    setMainState({ ...mainState, [variantName]: { ...mainState[variantName], [name]: checked } });
  };
                  ...
                  <td className="">
                    <div className="d-flex justify-content-center gap-3">
                      ...
                      <div className="edit-del red" onClick={() => deleteGroup(a.id)}>
                        {/* <img width={15} style={{ color: "yellow" }} height={15} src={delet} alt="" /> */}
                        {/* <i className="red1 ph-trash" style={{fontSize:'18px'}}  ></i> */}
                        <DeleteIcon />
                      </div>
                    </div>
                  </td>
...
//!important
// var resultNameList = resultpermissions.map((curr) => curr.name);
//  resultNameList = [ "Can change role", "Can delete role", "Can view role", "Can add products", "Can change products", "Can delete products", "Can view products" ]
// var resultNameListDict = resultNameList.map((curr) => ({
//   [curr.split(" ")[2]]: {
//     [curr.split(" ")[1]]: true,
//   },
// }));
